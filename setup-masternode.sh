...
function check_port() {
    for NEXTPORT in $(seq $STARTINGRPCPORT $ENDINGRPCPORT);
    do
...
function start_daemon() {
	...
	echo -e "${RED}Something is not right the daemon did not start. Will exit out so please log back in and run the script again by entering: ./setup-masternode.sh${NC}"
	exit
    fi
}
...
function status_loop() {
	...
	NUM='30'
	MSG1="${CYAN}Refreshes every 30 seconds until your Masternode finishes syncing to the Masternode list and will stop the loop on it's own.${NC}"
	MSG2="\e[2K\r"
	spinning_timer
